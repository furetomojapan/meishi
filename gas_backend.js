 ...
 *
 * 【デプロイ】v4.4以降は自動。このファイルを変更して main に push するだけで
 * GitHub Actions（gas-deploy.yml）が Apps Script への反映と本番デプロイ更新を行う。
 * ※ シート列が増える変更のみ、Apps Script エディタで initSheets() を1回手動実行
 */
...
const TRIAL_DAYS = 7; // v4.6: 新規登録者のPRO+＋G無料お試し日数
const TRIAL_MS   = TRIAL_DAYS * 24 * 60 * 60 * 1000;

// v4.5: テーマカラー（カード画面全体）。フロント THEME_OPTIONS と同期すること
const THEME_KEYS      = ["", "pink", "red", "orange", "green", "blue", "purple", "black"];
const FREE_THEME_KEYS = ["", "blue", "green"];
// 不正キー・FREEのPRO限定色は "" に矯正（profileはそのまま返すが themeColor のみ上書き）
function clampThemeColor(profile, isProPlan) {
  if (!profile || typeof profile !== "object") return profile;
  const key = String(profile.themeColor || "");
  const allowed = isProPlan ? THEME_KEYS : FREE_THEME_KEYS;
  if (allowed.indexOf(key) >= 0) return profile;
...
const ROUTES = {
  ...
  save_user_profile: { auth: "session", write: true, handler: (p) => {
    ...
    let profile = clampThemeColor(p.profile, isProPlan); // v4.5: FREEのPRO限定テーマ色は "" に矯正
    if (!isProPlan) {
      // v4.6: トライアル期限切れ後の保存でも、PRO時代の非表示データ（2件目以降の
      // リンク・PRO限定テーマ色）はシート上に温存する（PRO購入で復活）
      const stored = parseJson(f.row[3], []);
      if (stored.length > linkLimit) {
        while (links.length < linkLimit) links.push({ label: "", url: "" }); // 位置を保つ
        links = links.concat(stored.slice(linkLimit));
      }
      const storedProfile = parseJson(f.row[5], null);
      const storedColor   = storedProfile ? String(storedProfile.themeColor || "") : "";
      const storedIsProOnly = storedColor !== "" &&
        THEME_KEYS.indexOf(storedColor) >= 0 && FREE_THEME_KEYS.indexOf(storedColor) < 0;
      const incomingColor = String((p.profile && p.profile.themeColor) || "");
      // FREEで選べる色を明示的に選んだ場合のみ上書き。それ以外は保存済みPRO色を温存
      if (profile && storedIsProOnly &&
          !(incomingColor !== "" && FREE_THEME_KEYS.indexOf(incomingColor) >= 0)) {
        profile = JSON.parse(JSON.stringify(profile));
        profile.themeColor = storedColor;
      }
    }
    if (!saveUserProfile(p.name, p.displayName, links, profile))
  ...
  admin_set_trial: { auth: "admin", write: true, handler: (p) => {
    const days = Number(p.days) || 0;
    const end  = days > 0 ? Date.now() + days * 24 * 60 * 60 * 1000 : 0;
    if (!setTrialEnd(p.name, end))
...
// ── トライアル（v4.6）──────────────────────────────────────────────
function rowTrialEndMs(row, t) {
  if (t.colTrialEnd < 0) return 0;
  const v = row[t.colTrialEnd];
  if (!v) return 0;
  const ms = v instanceof Date ? v.getTime() : new Date(String(v)).getTime();
  return isNaN(ms) ? 0 : ms;
}

function setTrialEnd(name, ms) {
  const t = getUsersTable();
  if (t.colTrialEnd < 0) return false; // 列がない（initSheets未実行）
  const f = findUserRow(name);
  if (!f) return false;
  t.sheet.getRange(f.idx, t.colTrialEnd + 1).setValue(ms ? new Date(ms).toISOString() : "");
  invalidateUsersCache();
  return true;
}
...
function getAllUsersAdmin() {
  ...
  for (let i = 1; i < t.rows.length; i++) {
    ...
    // v4.6: 管理者には実効値ではなく生データを返す（誤って実効proを永続保存しないため）
    u.plan     = row[4] || "free";
...
// ── 管理者: ユーザー作成 ─────────────────────────────────────────
// v4.6: trialEndMs指定時はトライアル期限もセット（自己登録から使用）
function adminCreateUser(name, trialEndMs) {
  const t = getUsersTable();
  ...
  invalidateUsersCache();
  if (trialEndMs) setTrialEnd(name, trialEndMs);
...
function selfRegister(p) {
  ...
  adminCreateUser(userId, now.getTime() + TRIAL_MS); // v4.6: 7日間PRO+＋Gトライアル付与
...
// v4.6: 実効プラン — トライアル中はpro扱い（リンク・タグ・テーマの上限判定に使用）
function getUserPlan(name) {
  const f = findUserRow(name);
  if (!f) return "free";
  if (rowTrialEndMs(f.row, f.t) > Date.now()) return "pro";
  return f.row[4] || "free";
}
...
function findUsersByTag(tag, excludeName) {
  ...
  for (let i = 1; i < t.rows.length; i++) {
    ...
    const plan = rowTrialEndMs(row, t) > Date.now() ? "pro" : (row[4] || "free"); // v4.6: トライアル考慮
